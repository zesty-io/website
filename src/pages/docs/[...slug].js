...
// stripGitbookMeta
const stripGitbookMeta = (gitbookMarkdown) => gitbookMarkdown.replace(/---(.*?)---/is,'');
...
const replaceImages = (gitbookMarkdown) => gitbookMarkdown.replaceAll('../.gitbook/','https://raw.githubusercontent.com/zesty-io/zesty-org/master/.gitbook/').replaceAll('(<','(').replaceAll('>)',')').replaceAll(/ \(([0-9]+)\)/g,'%20%28$1%29')
// fix urls with .md references
const cleanMarkdownURLS = (tocMarkdown) => tocMarkdown.replaceAll('.md','/').replaceAll('](','](/docs/')
            ...
            <Container>
                <Box sx={{ display: 'flex' }}>
                    ...
                    >
                            ...
                            <MuiMarkdown overrides={muiTOCOverrides}>{cleanMarkdownURLS(toc)}</MuiMarkdown>
                    ...
                    >
                        <MuiMarkdown inlineCodeColor='dodgerblue' overrides={muiContentOverrides}>{markdown}</MuiMarkdown>
                    </Box>
                </Box>
            </Container>
        </Main>
